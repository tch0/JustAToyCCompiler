...
有了符号表，才能在语义分析时做一下上下文相关的分析。由于这里语法分析、语义分析、代码生成是一起做的，所以符号表的使用贯穿在整个扫描解析过程中。
...
表达式将会使用运算符优先级分析法进行分析。关于优先级分析的更多细节可以参考这里：[运算符优先级分析法](https://github.com/tch0/notes/blob/master/OperatorPrecedenceParser.md)，以及代码注释。
...
```
...
[intructions that put all paramter into stack in order]
