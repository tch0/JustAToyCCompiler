...
- [扩展c4](#%E6%89%A9%E5%B1%95c4)
  ...
  - [浮点数支持](#%E6%B5%AE%E7%82%B9%E6%95%B0%E6%94%AF%E6%8C%81)
    - [总体思考](#%E6%80%BB%E4%BD%93%E6%80%9D%E8%80%83)
...
按照整数参数的值执行代码。在需要按照整数值执行多个分支中的一个或数个之处使用，switch仅支持整数，其中的值应该是整数常量表达式，比如字符字面值，枚举等。按道理来说应该要在编译期计算，但现在并没有实现这样的机制，要么就运行时计算，同样也支持变量表达式，或者只支持常量。
...
```
线性列表`struct_symbols_list`保存所有的结构体信息。这些元素表示一个个类型，每个元素都是一个链表的首节点。节点中`next`域指向后续节点是它的成员按声明顺序存放。
...
单精度与双精度：
- C语言标准中是支持float单精度和double双精度的，当然还有long double这里不考虑。
- 最简单就是无论是float还是double都统一解析为同一个类型，不区分。和虚拟机钟int一个大小，在32位目标架构中就是32位也就是一般情况下float长度，64位目标架构中就是64位也就是一般情况下double的长度。

虚拟机与指令；
- 因为虚拟机并没有专门的浮点计算指令。需要设计并添加浮点指令支持。
- 可以考虑添加一个浮点寄存器，涉及浮点的操作就在该寄存器上，并且还应该在指令集中添加整型和浮点数互相转换的指令支持，因为同样的值在浮点和整型的二进制是不一样的，无法直接使用。浮点转整数就将值转换后移动到ax，反之如果类型提升了就移动到浮点寄存器中。
- 也可以考虑就存在ax中，虚拟机添加浮点类型和整数互相转换的指令完成转换，而不单独使用浮点寄存器。
- 存在ax中的选择的话会更简单一些，如果存在专门的浮点寄存器，每一次取操作数的时候，都要区分类型，然后看是从ax还是浮点寄存器取。而现有运算都是基于ax的，如果不支持新的基于浮点寄存器进行运算的指令的话。就需要生成大量将值从ax取到浮点寄存器的指令。
- 需要注意在栈和寄存器中，浮点数存在了int数组中，应该忠实地存储其浮点表示的二进制位。类型提升和转换的操作将全部委托给虚拟机指令。
- 但是现有的指令集都是做的整数计算。
- 看来还是要为所有支持的运算实现浮点版本才行！

表达式与运算：
- 浮点类型只能进行算术运算`+ - * /`比较运算`== != > >= < <=`，而不能进行`%`、逻辑运算和位运算。
- 支持后双目运算符中不支持浮点的需要进行类型检查并报错。
- 在此基础上对支持的运算符进行更严格的类型检查，限制能进行运算的操作符类型，排除比如对指针进行位运算这种明显的错误。

想了一想还是需要完整的浮点指令集才可行，可能要在虚拟机搞一个类似于CPU的浮点运算协处理器的东西来才行，待了解一下x86的浮点指令获取点灵感之后，如果可行的话再考虑支持。TODO。