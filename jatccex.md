...
- 标号不会占用任何代码空间，仅表示一个地址，用Value域就保存这个地址。
- 标号后接语句。
...
- 实现策略：
    - 标号的解析因为前面是标识符，和函数调用、变量不好区分，可以放到表达式`expression`解析过程中来解析，但标号定义并不是表达式，解析完应该立即返回，而普通表达式后面必须有`;`，所以使用标号就必须有分号。而且这会导致能用表达式的地方（比如if/while/for的表达式中）定义了标号都会被认为是语法正确。总体来说不是一个好的选择，也不怎么符合语义。
...
- 标号后必须有语句，在这里，它不应该位于变量声明前（标准C是可以的，只是要加分号），因为。同样如果位于块结尾，那么必须加上一个空语句`;`（同标准C语言一致）。
